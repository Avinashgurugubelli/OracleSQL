...
SELECT department_id, Count(employee_id) as "no of employees" FROM employees GROUP BY department_id;
...
*/
SELECT employee_id, first_name, last_name, Count(employee_id) FROM employees GROUP BY department_id; -- error
...
*/
SELECT job_id, department_id, Count(employee_id) FROM employees GROUP BY department_id, job_id;
SELECT department_id, Count(employee_id) FROM employees GROUP BY department_id, job_id;
...
*/
select max(salary), job_id, department_id from employees GROUP BY department_id, job_id;
...
*/
select max(salary), min(salary), job_id, department_id from employees GROUP BY department_id, job_id;
...
*/
SELECT SUM(Salary), AVG(MAX(salary)) FROM employees GROUP BY department_id HAVING SUM(Salary) > 90000;

SELECT department_id, SUM(Salary) FROM employees WHERE Salary > 1000 GROUP BY department_id HAVING Count(employee_id) > 1;
-- Group by errors:
-- 01. Aggregate functions cannot be used in WHERE clause.
SELECT first_name, Salary, department_id FROM Employees E1 WHERE Salary = MAX(Salary); -- error
SELECT  salary, max(Salary), department_id FROM Employees E1 group by department_id, salary having Salary = MAX(Salary);

-- 02. Aggregate functions cannot be used in WHERE clause even if GROUP BY is used.
SELECT department_id, SUM(Salary) FROM Employees WHERE SUM(Salary) > 2000 GROUP BY department_id; -- error
SELECT  salary, max(Salary), department_id FROM Employees E1 group by department_id, salary having Salary > 2000;
...
SELECT department_id, SUM(Salary) FROM Employees GROUP BY department_id ORDER BY job_id; -- errror;
...
SELECT  salary, ROUND(salary, 2), max(Salary), department_id FROM Employees E1 group by department_id, salary having Salary > 2000;
